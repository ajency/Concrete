  ...
  Variants.prototype = Object.assign({}, Variants.prototype, {
    ...
    _updateImages: function(variant) {
      ...
      if (variant.featured_image) {
        ...
        $('.product-image-slider').slick('slickGoTo', variantImagePos);
      }
  ...
  Cart.prototype = Object.assign({}, Cart.prototype, {
    _setupCartTemplates: function() {
      ...
      this.itemDiscountTemplate = this.itemTemplate
        ...
        .cloneNode(true);
      console.log(itemDiscountTemplate);
  ...
  Product.prototype = Object.assign({}, Product.prototype, {
    ...
    _liveRegionText: function(variant) {
      ...
      if (this.productState.onSale) {
        ...
        saveLabel = theme.strings.sale;
        savePrice = theme.Currency.formatMoney(
          variant.price,
          theme.moneyFormat
        );
      }
    ...
    _updatePriceComponentStyles: function(evt) {
      ...
      var variant_desc_data = $('input[name = variant_description]').val();
      if(variant_desc_data){
        variant_desc_data.split(';;').forEach(function(variant_descs) {
          var variant_descs_array = variant_descs.split(';');
          if(variant.id == variant_descs_array[0].trim()){
            //console.log(variant_descs_array[1]);
            $('.description').text(variant_descs_array[1].trim());
          }
        })
      }
    ...
    _updatePrice: function(evt) {
      ...
      var savePrice = this.priceContainer.querySelector(
...
function removeImageLoadingAnimation(image) {
  ...
  if (imageWrapper) {
    ...
    $('.hamburger').on('click', function(){
      $('.menu-list').addClass('expand').css("display", "block");
      $(".mobile-menu").css("left", "0");
      $(".main-child-menu.show").css("left", "0px");
      $('.main-child-menu.show ul.submenu').show();
    ...
    $('.close').on('click', function(){
      $('.menu-list').removeClass('expand').css("display", "none");
      $('.hamburger').css("background-color", "#FFF");
      $('.sub-megamenu').removeClass('show expand').css("display", "none");
      $('ul.dropdown-main-menu li:first-child').removeClass('active-item');
    });


    $(document).on("click", function(e){
      if( 
        $(e.target).closest(".sub-megamenu").length == 0
      ){
        $('.sub-megamenu').removeClass('expand show').css('display', 'none');
        $('.sub-megamenu').css("background-color", "#FFF");
        $('.megamenu').removeClass('active-item');
      }
    });

   $(document).on("click", function(e){
      if( 
        $(e.target).closest(".user-account").length == 0 &&
        $(".user-account").hasClass("open-acc") &&
        $(e.target).closest(".user").length == 0
      ){
        $('.user-account').removeClass('d-block');
      }
    });
    $('.user').on('click', function(){
      $(".user-account").toggleClass('d-block d-none');
      $(".user-account").addClass('open-acc');
    });
    $('#filterList li:first > a').addClass('active');

    $(document).ready(function() 
    {
          $('.close-icon').on('click', function(){
          $(".mobile-menu").css("left", "-390px");
          $(".main-child-menu.show").css("left", "-390px");
          $('ul.submenu').hide();
      ...
      $('.testimonials-slider').on('init', function (event, slick, direction) {
	    if (!($('.testimonials-slider .slick-slide').length > 4)) {
      ...
      $('.tag-slider').on('init', function (event, slick, direction) {
	    if (!($('.tag-slider .slick-slide').length > 5)) {
          $('.slick-dots').hide();
        }
      });
      $('.tag-slider').slick({
      ...
      $('.product-faq-content .card p').on('click', function(e){
        if($(this).hasClass('active')){
          $(this).toggleClass('active');
          $('.product-faq-content .card p').siblings().slideUp('slow');
    //      $(this).siblings().slideToggle('slow');
        }
          else{
              $('.product-faq-content .card p').removeClass('active');
              $('.product-faq-content .card p').siblings().slideUp('slow');
              $(this).toggleClass('active');
              $(this).siblings().slideToggle('slow');
          }
      ...
      $(".all-artciles .article-card").slice(0, 3).show();
      $("#loadmore").on("click", function(e){
        e.preventDefault();
        $(".all-artciles .article-card:hidden").slice(0, 3).slideDown(200, 'swing');
        if($(".all-artciles .article-card:hidden").length == 0) {
...
// });
    $('.mweb-mobile-menu > li').click(function(e) {
      ...
      if($(this).hasClass('show')){
        ...
        $(this).addClass('show');
        $('ul.submenu').show();
        $('.mweb-mobile-menu > li').removeClass('active-menu');
        $(this).addClass('active-menu');
      }
      $('.grandchild-submenu').hide();
    });


    
    $('.mweb-mobile-menu > li > ul.submenu > li.child-menu').click(function(e) {
      ...
      if($(this).hasClass('show')){
        ...
        $('ul.submenu > li.child-menu').removeClass('show');
        $(this).toggleClass('show');
      }
      $('ul.submenu > li > ul').not($el).slideUp();
...
$('ul.dropdown-main-menu li:first-child').addClass('megamenu');


$("ul.dropdown-main-menu li:first-child").hover(function(){
	
  $("ul.sub-megamenu").addClass("show");
  $('.sub-megamenu').addClass('expand').css("display", "block");
  $(this).addClass("active-item");
  $('.sub-megamenu li').first().addClass("showDropdown");
  $('.sub-megamenu li').first().addClass('active-menu');
});

$("ul.dropdown-main-menu li").not(':first').hover(function(){
  $('.sub-megamenu').removeClass('expand show').css('display', 'none');
  $('ul.dropdown-main-menu li:first-child').removeClass('active-item');
});

$(".sub-megamenu  > li").hover(function(){
  jQuery('.sub-megamenu  > li').removeClass("showDropdown");
  jQuery(this).addClass("showDropdown");
  $('.sub-megamenu  > li').removeClass('active-menu');
  $(this).addClass('active-menu');
});
